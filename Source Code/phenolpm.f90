subroutine phenolpm(ctrl, aepa,antes,antss,pdepth,bhfwsf,boots,cliname,cname,daa,dae,dap,daynum,ddae,ddap, &
                  & dgdde,dgdds,drs,dummy2,emrgflg,ems,endphenol,first7,fps,gdda,gdde,gdds,gddwsf,gmethod, &
                  & heads,hrs,ies,joints,lnpout,mats,partcoefleaf,partcoefstem,partcoefrepro,pchron,pdate, &
                  & seedbed,srs,tis,tss,useupgmpart,year)
!
...
!debe add items to print to the output file phenol.out
  ...
  write (ctrl%handles%luophenol,1500) pdatearr(1),pdatearr(3),pdatearr(4),ems(1),ems(3),     &
...
! debe added writing out and formatting of the gddwsf array
  write (ctrl%handles%luophenol,1600) gddwsf(1,1),gddwsf(1,2),gddwsf(1,3),gddwsf(1,4),       &
