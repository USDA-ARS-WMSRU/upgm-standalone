    ...
    !
    !
    subroutine crop_endseason(ctrl, bio, prevbio, biotot, bnslay, &
        & mature_warn_flg)
    ...
    ! Arguments
    !
    type(controls), intent(in) :: ctrl              
    type(biomatter), intent(in) :: bio
    type(biomatter), intent(in) :: prevbio 
    type(biototal), intent(in) :: biotot        
    integer(int32), intent(in) :: bnslay
    integer(int32), intent(in) :: mature_warn_flg
    
    ! Local variables
    !
    type(date) :: harvdate                          ! the date printed in the season.out file
    integer(int32) :: lay                           !
    real(sp) :: bg_stem_sum                         ! sum of below ground stem
    real(sp) :: hi                                  ! harvest index. yield/totbiomass
    real(sp) :: hui                                 ! heat unit index (ratio of acthucum to acthum)
    real(sp) :: root_fiber_sum                      ! sum of root fiber
    real(sp) :: root_store_sum                      ! sum of root storage
    real(sp) :: totbiomass                          ! the final total biomass.
    real(sp) :: yield                               ! the calculated yield of the crop.
    
    ! crop_endseason start
    !
    call caldat(ctrl%sim%juldate,harvdate%day,harvdate%mon,harvdate%year)
    ...
    if ((bio%database%thum>0.0).and.(bio%upgm%phenolflg==0)) then
        hui =  prevbio%growth%thucum/bio%database%thum
        ! print*, 'in crop_endseason hui = ', hui
    else if ((bio%database%thum>0.0).and.(bio%upgm%phenolflg==1).and.(bio%upgm%mats(1)/=999)) then
        hui = 1.0
        ! print*, 'in crop_endseason hui = ', hui
    else
        hui = 0.0
    ...
    if ((bio%growth%am0cfl>=0)) then            ! always print this one now - lew
        bg_stem_sum = 0.0
        root_store_sum = 0.0
        root_fiber_sum = 0.0
        do lay = 1, size(prevbio%mass%stemz)
            bg_stem_sum = bg_stem_sum + prevbio%mass%stemz(lay)
            root_store_sum = root_store_sum + prevbio%mass%rootstorez(lay)
            root_fiber_sum = root_fiber_sum + prevbio%mass%rootfiberz(lay)
        end do
        ...
        !  crop.
        yield = prevbio%mass%standstore*bio%database%ycon*prevbio%geometry%grainf
        ...
        totbiomass = prevbio%mass%standleaf + prevbio%mass%standstem + prevbio%mass%standstore +              &
            & prevbio%mass%flatleaf + prevbio%mass%flatstem + prevbio%mass%flatstore
        ...
        hi = yield/(totbiomass*bio%database%ycon)

        write (ctrl%handles%luoseason,1000) harvdate%year,prevbio%mass%standstem,prevbio%mass%standleaf,prevbio%mass%standstore,      &
            & prevbio%mass%flatstem,prevbio%mass%flatleaf,prevbio%mass%flatstore,bg_stem_sum,&
            & root_store_sum,root_fiber_sum,prevbio%geometry%zht,prevbio%geometry%dstm,        &
            & prevbio%geometry%zrtd,prevbio%geometry%grainf,biotot%xstmrep,prevbio%growth%dayap,             &
            & prevbio%growth%tchillucum, prevbio%growth%thucum,bio%database%thum,hui,bio%growth%dayam,          &
            & bio%growth%dayspring,bio%bname,yield,bio%database%ynmu,totbiomass,hi,bio%upgm%ies(1),  &
            & bio%upgm%joints(1),bio%upgm%boots(1),bio%upgm%heads(1),bio%upgm%antss(1),bio%upgm%mats(1),bio%upgm%hrs(1)
    end if

    ! for annual crops, always write out warning message
    ! if harvested before maturity
    if ((hui<1.0).and.(mature_warn_flg>0).and.                                      &
        & ((bio%database%idc==1).or.(bio%database%idc==2).or.(bio%database%idc==4).or.(bio%database%idc==5)))                 &
        & write (unit=6,fmt='(1x,3(a),i2,''/'',i2,''/'',i2,a,f5.1,a,a)') 'warning: ', &
        & bio%bname(1:len_trim(bio%bname)),' harvested ',harvdate%day,harvdate%mon,harvdate%year,' only reached ',         &
