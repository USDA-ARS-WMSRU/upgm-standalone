...
!          dae(r), dap(r), daynum(r), ddae(20)(c), ddap(20)(c),
...
!     first7 - used to set the value of aepa the first time phenolsb is called.
...
! filled with the awn initials information on the row for awn initials
! (row 7) and row 5 will be skipped because fps is on that row and
! filled with zeros.  need to read in value in row 5 in the dummy2 array.
  row = 7
  i = 5
!debe changed the following code to add rows 3 and 4 to row 7 in gddwsf array.
  call water_stress(adjgdd,bhfwsf,dummy2,gddwsf,row,i)
...
! to row 6.  the aifs stage needs to be in row 7.
...
!  growth stage.
!debe combined this stage with awn initials formed.
...
! rows 3 and 4 because row 7 is for awn initials formed and row 8 is for
! jointing.
...
! rows 3 and 4 because row 7 is for awn initials formed, row 8 is for
...
! to rows 3 and 4 because row 7 is for awn initials formed, row 8 is for
...
! array to rows 3 and 4 because row 7 is for awn initials formed, row 8
...
!debe add items to print to the output file phenol.out
  write (ctrl%handles%luophenol,1200) cname,cliname,pdepth,pdatearr(1),pdatearr(3),          &
                       & pdatearr(4),gmethod,emrgflg,seedbed,bhfwsf
 
  write (ctrl%handles%luophenol,1300) pdatearr(1),pdatearr(3),pdatearr(4),ems(1),ems(3),     &
                       ...
                       & /pchron,aifs(1),aifs(3),aifs(4),ddap(5),ddae(5),       &
                       & dgdds(5),dgdde(5),dgdde(5)/pchron,joints(1),joints(3), &
                       & joints(4),ddap(8),ddae(8),dgdds(8),dgdde(8),dgdde(8)   &
                       & /pchron,boots(1),boots(3),boots(4),ddap(9),ddae(9),    &
                       & dgdds(9),dgdde(9),dgdde(9)/pchron,heads(1),heads(3),   &
                       & heads(4),ddap(10),ddae(10),dgdds(10),dgdde(10),dgdde(9)&
                       & /pchron,antss(1),antss(3),antss(4),ddap(11),ddae(11),  &
                       & dgdds(11),dgdde(11),dgdde(9)/pchron,antes(1),antes(3), &
                       & antes(4),ddap(12),ddae(12),dgdds(12),dgdde(12),dgdde(9)&
                       & /pchron,mats(1),mats(3),mats(4),ddap(13),ddae(13),     &
                       & dgdds(13),dgdde(13),dgdde(9)/pchron,hrs(1),hrs(3),     &
                       & hrs(4),ddap(14),ddae(14),dgdds(14),dgdde(14),dgdde(9)  &
                       & /pchron
 
! debe added writing out and formatting of the gddwsf array
  write (ctrl%handles%luophenol,1400) gddwsf(1,1),gddwsf(1,2),gddwsf(1,3),gddwsf(1,4),       &
...
1400 format (/2x,39x,'gddwsf array',/1x,'phenological event',12x,'gn gdd',2x,  &
            ...
            &'row 16',25x,f5.1,3x,f5.1,3x,f3.1,4x,f3.1,6x,f3.1,/1x)
!
end subroutine phenolsb
