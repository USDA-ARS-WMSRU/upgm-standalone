subroutine phenolsw(ctrl, aepa,antes,antss,pdepth,bhfwsf,boots,cliname,cname,daa,dae,dap,daynum,ddae,ddap,dgdde, &
                  & dgdds,drs,dummy2,emrgflg,ems,endphenol,first7,fps,gdda,gdde,gdds,gddwsf,gmethod,heads,hrs,   &
                  & ies,joints,lnpout,mats,partcoefleaf,partcoefstem,partcoefrepro,pchron,pdate,seedbed,srs,tis, &
                  & tss,useupgmpart,year)
!
...
!     first7 - used to set the value of aepa the first time phenolsw is called.
...
!debe accumulate gdd simultaneously with that accumualted for tis stage.
...
! spikelet (row 7) and row 5 will be skipped because fps is on that row and
...
!  growth stage.
!debe moved this to be part of the code for tss because both stages can
...
! because row 5 was left filled with zeros in the fps stage.
  if (gdde>=(gddwsf(3,5)+gddwsf(4,5)+gddwsf(6,5))) then
     ...
     dgdde(6) = gdde
     print *,'ies = ',ies
     if ((tss(1).ne.999).and.(ies(1).ne.999)) then ! is this the right place for this if block?
         partcoefleaf = 0.3
         partcoefstem = 0.4
         partcoefrepro = 0.3
     endif
...
!        i = 12         no row in dummy2 array corresponds to antes.
!debe changed following code to add the correct rows in gddwsf array as
! noted above.  anthesis ends has the same additions as anthesis starts with
