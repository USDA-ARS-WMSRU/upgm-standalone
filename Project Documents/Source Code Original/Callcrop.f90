SUBROUTINE callcrop(aepa,aifs,daysim,sr,antes,antss,blstrs,boots,browns,       &
                      & callgdd,canht,canopyflg,cliname,cots,cropname,         &
...
!
REAL :: aepa,canht,dayhtinc,ecanht,gddtbg,maxht,pchron,tbase,toptlo,toptup,tupper
LOGICAL :: callgdd
...
IF ((ac0shoot(sr).LE.0.0).OR.(acdpop(sr).LE.0.0)) am0cgf = .FALSE.
!
...
!  print*, 'in callcrop just before call to crop seedsw = ', seedsw
  CALL crop(nslay(sr),aszlyt(1,sr),aszlyd(1,sr),asdblk(1,sr),asfcce(1,sr),      &
          ...
          & dummy2,ears,ecanht,egdd,emrgflg,ems,endlgs,epods,ergdd,eseeds,&
          & first7,fps,fullbs,gddtbg,germgdd,germs,ggdd,gmethod,gpds,           &
          & growth_stress,halfbs,heads,hrs,icli,ies,ies2,infls,joints,lf12s,    &
          & lf1s,lf2s,lf3s,lf4s,lf8s,mats,maxht,mffls,milks,mpods,mseeds,opens, &
          & pchron,pd,phenolflg,pm,py,seedsw,silks,soilwat,srs,tbase,tis,toptlo,&
          & toptup,tsints,tss,tupper,wfpslo,wfpsup,yelows)
! print*, 'in Callcrop just after call to Crop, seedsw = ', seedsw
! print*, 'in Callcrop justa after call to Crop, silks(1) = ', silks(1)
...
IF ((atmstandstem(sr)+atmstandleaf(sr)+atmstandstore(sr)+atmflatstem(sr)        &
  & +atmflatleaf(sr)+atmflatstore(sr)).GT.0.0) THEN
