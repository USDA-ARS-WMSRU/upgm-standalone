...
! e.g. wsf = 1.0.
IF (x.GT.wsflo) THEN   ! wsflo = 0.8
  gddwsf(row,5) = dummy2(i)   ! = to the gn values
ELSE IF (x.LT.wsfhi) THEN  ! wsfhi = 0.4
  gddwsf(row,5) = dummy2(i+n)     ! = to the gs values
!debe x is between lo and hi wsf values
ELSE IF (dummy2(i).EQ.dummy2(i+n)) THEN    !gn and gs value is the same
  adjgdd = dummy2(i)
  gddwsf(row,5) = adjgdd
ELSE IF (dummy2(i).GT.dummy2(i+n)) THEN    ! gn value is > gs value, a + slope
   !code a for equation
  ...
  adjgdd = y
  gddwsf(row,5) = adjgdd
ELSE IF (dummy2(i).LT.dummy2(i+n)) THEN     !gn value is < gs value, a - slope
   !code b for equation
  m = ((gddwsf(row,1)-gddwsf(row,2))/(gddwsf(row,4)-gddwsf(row,3)))
