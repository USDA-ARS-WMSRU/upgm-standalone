...
! Local variables
!
REAL :: apar,arg_exp,bhfwsf_adj,clfarea,clfwt,ddm,ddm_rem,dht,dlfwt,drfwt,drpwt,&
      & drswt,dstwt,eff_lai,ff,ffa,ffr,ffw,frst,gif,huf,hufy,hui0f,hux,         &
      & lost_mass,par,parea,pcht,pchty,pddm,pdht,pdiam,pdrd,prd,prdy,p_lf,      &
      & p_lf_rp,p_rp,p_rw,p_st,stem_propor,strs,temp_fiber,temp_sai,temp_stem,  &
      & temp_stmrep,temp_store,trad_lai,ts,wcg,wffiber,wfstore,wmaxd,xw
INTEGER :: day,doy,i,irfiber,irstore,mo,yr
INTEGER :: dayear
REAL :: strsdayhtinc, temps
...
!     strs - stress factor (fraction of growth occuring accounting for
!            stress)
...
!print*, 'winter_ann_root = ', winter_ann_root
...
ELSEIF ((phenolflg .EQ. 1) .AND. (boots(1) .EQ. 999)) THEN
        arg_exp = -(hui-bc0clf)/bc0dlf
    IF (arg_exp.GE.max_arg_exp) THEN
      p_lf = bc0alf + bc0blf/max_real
    ELSE
...
IF ((cook_yield.EQ.1).AND.(bcyld_coef.GT.1.0).AND.(bcresid_int.GE.0.0).AND.     &
  & ((bchyfg.EQ.0).OR.(bchyfg.EQ.1).OR.(bchyfg.EQ.5)))                          &
...
ELSE IF ((canopyflg.EQ.1).AND.(antss(1).EQ.999)) THEN     !use canht from phenologymms method
...
! emergence if WEPS has not also arrived at emergence.
    ...
    END IF
      ...
      ! until shoot breaks surface, no solar driven growth
      ! call it lack of light stress
    IF (canht.LE.0.0) strs = 0.0

   ! print *,'canht = ', canht, 'strs = ', strs
...
DO i = 1,irfiber
  IF (i.LE.irstore) bcmrootstorez(i) = bcmrootstorez(i) + (drswt*wfl(i)/wfstore)
...
IF (am0cfl.GE.1) THEN
  ...
  WRITE (luocrop,1000) daysim,doy,yr,bcdayap,hui,bcmstandstem,bcmstandleaf,     &
                     ...
                     & hu_delay,temp_sai,temp_stmrep,bc0nam,gddday
END IF
! 
 1000 FORMAT (1x,i5,1x,i3,1x,i4,1x,i4,1x,f6.3,12(1x,f7.4),1x,f7.2,3(1x,f7.4),   &
            & 8(1x,f6.3),1x,e12.3,10(1x,f6.3),2(1x,f8.5),' ',a20,f8.3)

   WRITE (luocanopyht,1100) daysim,doy,day,'/',mo,'/',yr,canopyflg,dht,strs,    &
            & bczht,dayhtinc,strsdayhtinc,canhty,canht,antss(1) 

 1100 FORMAT (2x,i3,2x,i3,2x,i2,a1,i2,a1,i2,5x,i1,5x,f6.4,2x,f6.4,2x,f6.4,2x,f6.4,5x,f6.4,4x,     &
            & f12.8,4x,f12.8,5x,i3)
