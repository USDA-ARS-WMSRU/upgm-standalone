...
INTEGER :: bc0idc,bc0transf,bcbaflg,bcdayam,bcdayap,bcdayspring,bchyfg,bctdtm,  &
         ...
         & first7,gmethod,growth_stress,icli,pd,phenolflg,pm,py,seedsw           ! debe added phenolflg 
...
REAL :: bprevflatstore,bprevgrainf,bprevht,bprevhucum,bprevliveleaf,bprevmshoot,&
      ...
      & btxstmrep,btzht,bweirr,bwtdmn,bwtdmx,bwzdpt,cc0fd1,cc0fd2,dayhtinc,     &
      & ecanht,gddtbg,maxht,pchron,tbase,canht,toptlo,toptup,tupper
...
!
CHARACTER(80) :: cname
INTEGER,SAVE :: daa,dae,dap,dav,daynum,dd,lay,lncntr,mm,pdate,rowcntr,verns,yr, &
              & yy,tempsw
INTEGER :: dayear
REAL :: daylen,huc1
...
REAL,SAVE :: canhty,elong,gdda,gddday,gdde,gdds,gddv,hu_delay,nolvs,photo_delay,&
           & pot_leaf_mass,pot_stems,root_store_rel,tavgtemp,tmaxtemp,tmintemp, &
           & todayln,trend,vern_delay,yestln
...
IF (cropname.EQ.'corn') THEN
  cropname = 'corn'
ELSE IF (cropname.EQ.'drybeans') THEN
  cropname = 'dry beans'
...
! parameters file.
ELSE IF (cropname.EQ.'milletpearlforage') THEN
  cropname = 'hay millet'
ELSE IF (cropname.EQ.'milletfoxtailseed') THEN
  cropname = 'hay millet'
ELSE IF (cropname.EQ.'milletprosograin') THEN
  cropname = 'proso millet'
ELSE IF (cropname.EQ.'sorghum') THEN
  cropname = 'sorghum'
ELSE IF (cropname.EQ.'barleyspring') THEN
  cropname = 'spring barley'
ELSE IF (cropname.EQ.'wheatspring') THEN
  cropname = 'spring wheat'
ELSE IF (cropname.EQ.'sunflower') THEN
  cropname = 'sunflower'
ELSE IF (cropname.EQ.'barleywinter') THEN
  cropname = 'winter barley'
ELSE IF (cropname.EQ.'wheatwinter') THEN
...
IF (bm0cif) THEN
  CALL cinit(bnslay,bszlyt,bszlyd,bsdblk,bsfcce,bsfcec,bsfsmb,bsfom,bsfcla,     &
           ...
           & gdds,gddv,gddwsf,jan1,lnarray,lncntr,lnpout,nolvs,pdate,rowcntr,   &
           & seedsw,tempsw,todayln,verns,yestln,yr) 
...
ELSE
  ...
  IF ((trend.NE.0.0).AND.(bcthucum/bcthum.GT.bc0hue)) bcleafareatrend = trend
...
CALL phenolmms(aepa,aifs,antes,antss,bc0growdepth,bctmin,bhfwsf,blstrs,boots,   &
             ...
             & milks,mm,mpods,mseeds,nolvs,opens,pchron,pdate,rowcntr,seedsw,   &
             & silks,soilwat,srs,tbase,tis,todayln,toptlo,toptup,tsints,tss,    &
             & tupper,yelows,yestln,yy)
! 
!debe added dayhtinc to be able to pass the daily increase in height to growth
! for the ht_dia_sai subroutine in place of the weps/upgm variable dht when
! canopyflg = 1.
...
IF ((bcfleaf2stor.GT.0.0).OR.(bcfstem2stor.GT.0.0).OR.(bcfstor2stor.GT.0.0))    &
  & THEN
  IF ((bc0idc.EQ.2).OR.(bc0idc.EQ.5)) THEN
          ! check winter annuals for completion of vernalization,
          ! warming and spring day length
     IF ((bczgrowpt.GT.0.0).AND.(bctchillucum.GE.chilluv).AND.                  &
       & (bctwarmdays.GE.shoot_delay*bctverndel/verndelmax).AND.                &
       & (huiy.GT.spring_trig)) THEN
  ...
  ELSE IF (bc0idc.NE.7) THEN
          ...
          ! (most) leaf mass
     IF (bcleafareatrend.LT.0.0) THEN  ! last change in leaf area was a
                                       ! reduction
                                       ! ! 0.42 * 2 = 0.84
        IF (bcfliveleaf*bcmstandleaf.LT.0.84*bc0storeinit*bcdpop*mgtokg*        &
          & bcfleafstem/(bcfleafstem+1.0)) THEN ! below minimum living leaf mass
                                                !(which is twice seed leaf mass)
           IF (bctwarmdays.GE.shoot_delay) THEN ! enough warm days to start
                                                ! regrowth
              IF (bcthucum/bcthum.GE.bc0hue) THEN ! heat units past emergence
                                                  ! ! not yet mature
                 IF ((bcthucum.LT.bcthum).OR.((bc0idc.EQ.3).OR.(bc0idc.EQ.6)))  &
                   & THEN                                       ! perennial
...
! to 999. If phenolflg = 0, then the UPGM way will be used. When huiy is greater than or 
! equal to 1.0, maturity has been reached. This allows the possibility of using either
! approach to determine maturity.

  IF (((phenolflg .EQ. 1) .AND. (mats(1) .EQ. 999)) .OR. ((phenolflg .EQ. 0) &
      & .AND. (huiy .LT. 1.0))) THEN
...
! shoot_grow:
  IF ((emrgflg.EQ.1).AND.(ems(1).EQ.999).AND.(hui.GT.bcthu_shoot_beg)) THEN
!      if ((daysim .eq. 7) .or. (daysim .eq. 8)) then
!          bcthu_shoot_end = 0.06
!      else
!          bcthu_shoot_end = 0.05
...
! = bcthu_shoot_end in the call to shoot_grow, add 0.00001 to huiy below:
     IF (huiy.GE.bcthu_shoot_end) bcthu_shoot_end = huiy + 0.00001    !should 'then' be here?
  ...
  ELSE IF ((huiy.LT.bcthu_shoot_end).AND.(hui.GT.bcthu_shoot_beg)) THEN
     CALL shoot_grow(bnslay,bszlyd,bcdpop,bczmxc,bczmrt,bcfleafstem,bcfshoot,   &
                   & bc0ssa,bc0ssb,bc0diammax,hui,huiy,bcthu_shoot_beg,         &
                   & bcthu_shoot_end,bcmstandstem,bcmstandleaf,bcmstandstore,   &
                   & bcmflatstem,bcmflatleaf,bcmflatstore,bcmshoot,bcmtotshoot, &
 ...
 END IF ! ends the if emrgflg = 1 if block
...
! passed on to emerge where they are used. Added tempsw as the array index of these arrays.
  ...
  IF ((huiy.LT.bcthu_shoot_end).AND.(hui.GE.bcthu_shoot_end)) THEN
...
!          ! calculate plant growth state variables
  CALL growth(bnslay,bszlyd,bc0ck,bcgrf,bcehu0,bczmxc,bc0idc,bc0nam,a_fr,b_fr,  &
            ...
            & joints,mats)
            ! canht, cropname, dummy2, ems, gdde, ies, joints, lf4s, maxht)
