...
INTEGER :: bcdayap,day,doy,lay,mo
INTEGER :: dayear
REAL :: frac_lay
...
! local variables
!
!     the following subroutine arguments are not used: bczmrt  jcaii  8/08/08
...
!     shoot_hui - today fraction of heat unit shoot growth index
!                 accumulation
...
IF (end_shoot_len.LE.bczgrowpt) WRITE (UNIT=6,FMT='(1x,3(a),f7.4,a,f7.4,a)')    &
                                      & 'warning: ',bc0nam(1:len_trim(bc0nam)), &
                                      &' growth halted. shoot extension: ',     &
                                     & end_shoot_len,' depth in soil: ',        &
                                     & bczgrowpt,' meters.'
...
!End of moved section 
 IF (emrgflg .EQ. 0) WRITE (luoemerge,1000) cropname,doy,dap,pd,pm,py,         &
                       & bc0growdepth,ems(1),ems(4),ems(3),ems(2),seedbed,     &
                       & emrgflg,gddday,gddtbg,cliname, 'in Shoot_grow'
...
IF ((cook_yield.EQ.1).AND.(bcyld_coef.GT.1.0).AND.(bcresid_int.GE.0.0).AND.     &
  & ((bchyfg.EQ.0).OR.(bchyfg.EQ.1))) CALL cookyield(bchyfg,bnslay,dlfwt,dstwt, &
...
bcmflatstore = bcmflatstore + drpwt*(1.0-stem_propor)
      ...
      ! change
IF ((bcmstandleaf+dlfwt).GT.0.0) bcfliveleaf = (bcfliveleaf*bcmstandleaf+dlfwt) &
  & /(bcmstandleaf+dlfwt)
...
DO lay = 1,bnslay
  ...
  IF ((bcmrootstorez(lay).GT.0.0).AND.(d_s_root_mass.GT.0.0)) THEN
...
IF (am0cfl.GE.1) WRITE (luoshoot,1100) daysim,doy,yr,dap,shoot_hui,s_root_sum,  &
                                     & f_root_sum,tot_mass_req,end_shoot_mass,  &
                                     & end_root_mass,d_root_mass,d_shoot_mass,  &
                                     & d_s_root_mass,end_stem_mass,             &
                                     & end_stem_area,end_shoot_len,bczshoot,    &
