SUBROUTINE cinit(bnslay,bszlyt,bszlyd,bsdblk,bsfcce,bsfcec,bsfsmb,bsfom,bsfcla, &
               ...
               & lncntr,lnpout,nolvs,pdate,rowcntr,seedsw,tempsw,todayln,verns, &
               & yestln,yr)
...
!
! ***** leaf number *****
! variables: lnarray - not currently implemented, rowcntr, yestln, todayln,
!
! ***** canopy height *****
...
!
REAL :: alog
REAL :: bphu,bsa,dg,dg1,ephu,heat_unit,jreal,max_air,min_air,sphu,wt1,xz, &
      & yp1,ypn
INTEGER :: d,dn,dxx,hdate,i,j,m,mo,n,sdmn,sdmx,y
...
!     + + + local variables + +
!     character*20 cpnm
!     sdmn, sdmx declared here but commented out in the code   jcaii  8/08/08
...
END IF
DO i = 2,bnslay
  IF (((bszlyd(i-1)*mmtom.LT.bczrtd).AND.(bszlyd(i)*mmtom.GE.bczrtd))) THEN
              ! mg/plant * #/m^2 * 1kg/1.0e6mg = kg/m^2
...
IF (am0cfl.GT.0) WRITE (60,1100) pdate,hdate,bcthudf,dtm,bctdtm,phu,bcthum
